...
export default function DonorProfilePage() {
  ...
  async function fetchProfile() {
    try {
      setLoading(true)
      const res = await fetch("/api/v1/donor")
      const data = await res.json()
      if (!res.ok) throw new Error(data.error)
      setProfile(data.user)
      prefillForm(data.user)
    } catch (err: any) {
  ...
  function prefillForm(u: UserProfile) {
    setForm({
      fullName: u.fullName || "",
      phone: u.phone || "",
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Toast */}
      {toast && (
        <div className={`fixed top-4 right-4 z-50 flex items-center gap-2 px-4 py-3 rounded-xl shadow-lg text-sm font-medium transition-all ${toast.type === "success"
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 space-y-5">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            ...
            {!isEditing ? (
              <button
                ...
                className="flex items-center cursor-pointer gap-1.5 text-sm font-medium text-blue-600 hover:text-blue-700 transition"
              >
                <Pencil size={13} />
                Edit
              </button>
              ...
              <button
                ...
                className="flex items-center gap-1 cursor-pointer text-sm text-gray-400 hover:text-gray-600 transition"
              >
                <X size={14} />
                Cancel
              </button>
          ...
          <form onSubmit={handleSubmit}>
            ...
            {/* Submit — edit mode only */}
            {isEditing && (
              <div className="px-6 py-4 border-t border-gray-100">
                <button
                  type="submit"
                  disabled={saving}
                  className="w-full bg-blue-600 text-white text-sm font-medium py-2.5 rounded-xl hover:bg-blue-700 transition disabled:opacity-60"
                >
        ...
        {profile?.createdAt && (
          <p className="text-center text-xs text-gray-400">
            Donor since {formatDate(profile.createdAt)}
          </p>
        )}
      </div>
    </div>
