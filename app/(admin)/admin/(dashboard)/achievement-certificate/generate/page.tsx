...
const GenerateAchievement = () => {
  ...
  const handleGenerate = async () => {
    ...
    setIsLoading(true);
    try {
      await api.post("/achivement-certificate/generate", { memberId });
      toast.success("Achievement certificate generated successfully");
      router.push("/admin/achievement-certificate");
    } catch (error: any) {
  ...
  return (
    <div>
      <Header
        ...
        pageName="Dashboard"
        currentPage="Create Achievement Certificate"
      />
      ...
      <div className="p-6 bg-gray-50 h-[60vh] flex items-center">
        <div className="bg-white p-5 rounded-md lg:min-w-xl mx-auto w-full max-w-lg">
          <div className="mb-5">
            <h3 className="text-xl font-bold">Generate Achievement Certificate</h3>
            <p className="text-sm text-gray-400 mt-0.5">
              First choose a member for creating a certificate
            </p>
          </div>
          ...
          >
            {isLoading ? "Generating..." : "Create Achievement Certificate"}
          </Button>
        </div>
      </div>
    </div>
